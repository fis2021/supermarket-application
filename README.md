...
```git
git clone https://github.com/fis2021/supermarket-application
```
...
`$HOME/.supermarket-application/Users.db` (for Linux and MacOS) and `%USERPROFILE%/.supermarket-application/Users.db` file, using the [nitrite-explorer-3.4.3.jar](https://github.com/nitrite/nitrite-java/releases/download/v3.4.3/nitrite-explorer-3.4.3.jar), provided by Nitrite Java.
Download the jar and run `java --module-path $PATH_TO_FX --add-modules javafx.controls nitrite-explorer-3.4.3.jar


Choose the `$HOME/.supermarket-application/Users.db` and add `test` as both the username, and the password to access the database, then click `Open`.
